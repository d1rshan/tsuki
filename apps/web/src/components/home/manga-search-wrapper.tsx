...
export function MangaSearchWrapper({ children }: { children: ReactNode }) {
...
function SearchResults({
  ...
  const {
    ...
    isError,
    isDebouncing,
  } = useMangaSearch(searchQuery, includeNsfw);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {isLoading || (mangas.length === 0 && isPending) ? (
        <Loader variant="inline" className="h-64" />
      ) : isError ? (
        <ErrorState message="Failed to search manga" description="Please try again in a moment." />
      ) : mangas.length === 0 && !isPending ? (
        <EmptyState
