...
import type { Review, MangaReview } from "@/lib/types";
...
export function MangaReviewItem({ review }: { review: MangaReview }) {
  if (!review.manga) return null;

  const cover = getMangaCoverImage(review.manga);
  const title = getMangaTitle(review.manga);
  ...
  return (
    <article className="group py-8 first:pt-0 border-b border-border/40 last:border-0 flex gap-6 md:gap-8">
      ...
      <div className="flex-1 min-w-0">
        <div className="mb-5">
          <Link href={`/manga/${review.mangaId}`} className="inline-block">
            <h3 className="text-lg md:text-xl font-bold tracking-tight hover:text-primary transition-colors">
              {title}
            </h3>
          </Link>
