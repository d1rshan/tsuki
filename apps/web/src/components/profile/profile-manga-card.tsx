...
import type { MangaLibraryEntry } from "@/lib/types";

type Manga = NonNullable<MangaLibraryEntry["manga"]>;

interface ProfileMangaCardProps {
  manga: Manga;
  rating?: number | null;
  chaptersRead?: number | null;
}

export function ProfileMangaCard({ manga, rating, chaptersRead }: ProfileMangaCardProps) {
  const cover = getMangaCoverImage(manga);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-3 flex flex-col justify-end z-10 pointer-events-none">
        ...
        {chaptersRead != null ? (
          <div className="mt-1 text-xs text-white/80 font-medium">Ch. {chaptersRead}</div>
        ) : null}
      </div>
    </Link>
  );
