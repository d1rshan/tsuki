...
export function AnimeActions({ animeId }: AnimeActionsProps) {
  ...
  const entry = (activityRes?.data?.entry as LibraryEntry) ?? null;
  const review = (activityRes?.data?.review as Review) ?? null;
  ...
  if (isSessionPending || (isAuthenticated && isActivityLoading)) {
    return (
      <div className="flex gap-2">
        <Skeleton className="h-10 w-[140px] rounded-xl bg-foreground/10" />
        <Skeleton className="h-10 w-10 rounded-xl shrink-0 bg-foreground/10" />
      </div>
    );
  }
