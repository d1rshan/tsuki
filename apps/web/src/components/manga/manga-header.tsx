  ...
  return (
    <div className="relative z-10 -mt-20 flex flex-col gap-6 border-b pb-8 md:-mt-32 md:flex-row md:items-end md:gap-8">
      ...
      <div className="flex flex-1 flex-col gap-2 pb-2 md:pb-4">
        ...
        {manga.titleNative && manga.titleNative !== title && (
          <p className="font-medium text-muted-foreground">{manga.titleNative}</p>
        )}
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          {manga.format && <Badge variant="secondary">{manga.format}</Badge>}
          {manga.status && (
            <Badge variant="outline" className="uppercase text-muted-foreground">
              {formatEnum(manga.status)}
            </Badge>
          )}
          {manga.season && manga.seasonYear && (
