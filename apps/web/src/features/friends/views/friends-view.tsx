...
import type { DiscoveryUserSummary, FeedActivity } from "@tsuki/api/types";
...
type FeedType = "following" | "public";

async function getFeed(type: FeedType, cursor?: string | null) {
...
function Feed({ type }: { type: FeedType }) {
  const query = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => getFeed(type, pageParam),
  ...
  if (!activities.length)
    return (
      <EmptyState
        title={type === "following" ? "No Activity from people you Follow yet" : "No Activity yet"}
      />
    );
...
function Discover() {
  ...
  const query = useQuery({
    ...
    queryFn: () => getDiscovery(username),
    placeholderData: keepPreviousData,
  });
  const follow = useFollowMutation(async (relationship, target) => {
    client.setQueriesData<DiscoveryUserSummary[]>({ queryKey: friendsKeys.all }, (users) =>
      users?.map((user) => (user.username === target ? { ...user, relationship } : user)),
    );
  });
